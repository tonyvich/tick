...
) 
{
    ...
    // --- Users vars
    $scope.users                  = {};                     // Containing all the users registered in the application
    $scope.roles                  = [                       // Users roles
        { name: 'Admin'         , value: 'admin'},
        { name: 'Tasker'        , value: 'tasker'},
        { name: 'Subordinate'   , value: 'subordinate'}
    ]
    $scope.selectedUser           = {};                     // Model for the user in role assignment
    $scope.selectedRole           = {};                     // Model for the role in role assignment
    $scope.subordinateUsers       = [];                     // Contain users with a subordinate role
    ...
    /**
     * Set Task detail ( Bind data to detail section )
     * @param task_id ( Id of the task to bind )
     * @return 
     */

    $scope.setTaskDetail            = function( task_id ){
        $scope.content.display = "task";
        $scope.detail.display = "task";
        $.each($scope.tasks, function(i, task){
            if( task.id == task_id ){
                if( task.ends_at !== null){
                    date = new Date( task.ends_at );
                    $scope.detail.endTime = date.getTime();
                }
                $scope.detail.task = task;
            }
        });
    }
    ...
    $scope.getUsers = function (){
        usersResource.get().then( function ( returned ){
            $scope.users = returned.data.users;
            $.each( $scope.users, function( i, user){
                if( user.role == "subordinate" ){
                    $scope.subordinateUsers.push( user );
                }
            });
        });
    }
    ...
    $scope.updateUserRole = function(){
        var user  = {};
        user.id   = $scope.selectedUser;
        user.role = $scope.selectedRole;
        usersResource.update( user ).then( function ( returned ){
            if( returned.status == 200 ){
                $scope.getUsers();
                $scope.alertMessage( '#manage-users-message', 'success', 'The user has been successfully updated');
            } else {
                $scope.alertMessage( '#manage-users-message', 'error', 'The user has been successfully updated');
            }
        });
    }
